...
void cproxy_process_a2b_downstream_nread(conn *c) {
    ...
    if (c->substate == bin_reading_get_key &&
        ...
        item *it = item_alloc(key, keylen, flags, 0, NULL, vlen + 2, c->cksumlen);
        if (it != NULL) {
            c->item = it;
...
int a2b_multiget_skey(conn *c, char *skey, int skey_length, int vbucket, int key_index) {
    ...
    item *it = item_alloc("b", 1, 0, 0, NULL, sizeof(protocol_binary_request_get), 0);
    if (it != NULL) {
...
bool cproxy_forward_a2b_item_downstream(downstream *d, short cmd,
    ...
    if (c != NULL) {
        ...
        if (cproxy_prep_conn_for_write(c)) {
            ...
            item *it_hdr = item_alloc("i", 1, 0, 0, NULL, hdrlen, 0);
            if (it_hdr != NULL) {
